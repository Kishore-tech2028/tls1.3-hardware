...
// ============================================================================
// rx_record_fifo.sv
// Synchronous FIFO for RX record buffering (session_controller → record_layer)
// Ready/Valid handshake on both sides
// ============================================================================

import record_layer_pkg::*;

module rx_record_fifo (
    input  logic clk,
    input  logic rst_n,

    // Write Port (from session_controller RX)
    input  logic                write_en,
    input  logic [31:0]         write_data,
    output logic                write_ready,       // FIFO not full
    output logic                write_almost_full, // FIFO >80% full

    // Read Port (to record_parser)
    input  logic                read_en,
    ...
    always_comb begin
        ...
        if (write_en && write_ready) begin
            ...
            fifo_count_w = fifo_count_r + 1'b1;
        end
